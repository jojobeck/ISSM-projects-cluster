...
%mask has to be number of elements
%Process options
...
mask = getfieldvalue(options, 'mask', ones(md.mesh.numberofelements, 1));
if size(mask) ~= md.mesh.numberofelements,
    error('mask hast to be on elements not vertecies');
end
...
% pos1=(Range1(:,1)<level & Range1(:,2)>=level & mask ==1);
% pos2=(Range2(:,1)<level & Range2(:,2)>=level & mask ==1);
% pos3=(Range3(:,1)<level & Range3(:,2)>=level & mask ==1);
pos1=(Range1(:,1)<level & Range1(:,2)>=level & mask >0);
pos2=(Range2(:,1)<level & Range2(:,2)>=level & mask >0);
pos3=(Range3(:,1)<level & Range3(:,2)>=level & mask >0);
